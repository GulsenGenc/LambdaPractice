...
public class C5_Method_Referance_String {
    ...
    public static void buyukYaz(List<String> l) {
        l.stream().map(Methods::buyukYaz).forEach(Methods::stringleriYazdır);
    }

    //S2: Stringleri uzunluklarina gore siralayip yaziniz
    public static void sirala(List<String>l){
        l.stream().sorted(Comparator.comparing(String::length)).forEach(Methods::stringleriYazdır);
    ...
    public static void eIleBas(List<String>l){
        l.stream().filter(Methods::eIleBaslıyorMu).forEach(Methods::stringleriYazdır);
    }
